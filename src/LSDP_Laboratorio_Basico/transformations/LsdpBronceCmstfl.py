...
ruta_relativa_parquet = spark.conf.get("pipelines.parameters.rutaCompletaMaestroCliente")
ruta_relativa_schema = spark.conf.get("pipelines.parameters.rutaSchemaLocationCmstfl")

print("[LsdpBronceCmstfl] Inicializando modulo...")
...
print(f"  rutaCompletaMaestroCliente = {ruta_relativa_parquet}")
print(f"  rutaSchemaLocationCmstfl   = {ruta_relativa_schema}")
...
print(f"[LsdpBronceCmstfl] Ruta parquet CMSTFL : {ruta_parquet}")
print(f"[LsdpBronceCmstfl] Ruta schema location: {ruta_schema_location}")
...
campos_cluster_cmstfl = ["FechaIngestaDatos", "CUSTID"]


def transformar_cmstfl(df: DataFrame) -> DataFrame:
    """
    Logica pura de transformacion para la streaming table cmstfl.
    ...
    df_reordenado = reordenar_columnas_liquid_cluster(df_con_timestamp, campos_cluster_cmstfl)

    print(f"[LsdpBronceCmstfl] Reordenamiento de columnas exitoso. Schema: {df_reordenado.columns[:4]}...")
...
def tabla_bronce_cmstfl():
    """
    Streaming table cmstfl: ingesta incremental del parquet Maestro de Clientes AS400.
